...
func PrepareToCreateInObjectBuilder(ctx context.Context, conn *pgxpool.Pool, req *nb.CommonMessage, reqBody models.CreateBody) (map[string]any, []map[string]any, error) {
	var (
		fieldM     = reqBody.FieldMap
		tableSlugs = reqBody.TableSlugs
		fields     = reqBody.Fields
	...
	// * RANDOM_NUMBER
	{
		...
		if ok {
			for {
				...
				isExists, err := IsExists(ctx, conn, models.IsExistsBody{TableSlug: req.TableSlug, FieldSlug: randomNumbers.Slug, FieldValue: randNum})
				if err != nil {
					return map[string]any{}, []map[string]any{}, err
				}
	...
	// * RANDOM_TEXT
	{
		...
		if ok {
			for {
				randText := GenerateRandomString(cast.ToString(randomText.Attributes["prefix"]), cast.ToInt(randomText.Attributes["digit_number"]))
				isExists, err := IsExists(ctx, conn, models.IsExistsBody{TableSlug: req.TableSlug, FieldSlug: randomText.Slug, FieldValue: randText})
				if err != nil {
					return map[string]any{}, []map[string]any{}, err
				}
