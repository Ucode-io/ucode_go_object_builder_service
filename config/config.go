...
func Load() Config {
	...
	config.PostgresUser = cast.ToString(getOrReturnDefaultValue("POSTGRES_USER", "shokhrukh"))
	config.PostgresPassword = cast.ToString(getOrReturnDefaultValue("POSTGRES_PASSWORD", "12345"))
	config.PostgresDatabase = cast.ToString(getOrReturnDefaultValue("POSTGRES_DATABASE", "postgres_test"))
