	...
	for _, section := range sections {
		for _, fieldReq := range section.Fields {
			...
			if strings.Contains(fieldReq.Id, "#") {
				...
				err = conn.QueryRow(ctx, "SELECT id FROM view WHERE relation_id = $1", relation.Id).Scan(&viewOfRelation.Id)
				...
				if relation != nil {
					for _, fieldID := range viewFieldIds {
						...
						err := conn.QueryRow(ctx, "SELECT id, enable_multilanguage, slug FROM field WHERE id = $1", fieldID).Scan(&field.Id, &field.EnableMultilanguage, &field.Slug)
						if err != nil {
							return nil, err
						}
						if field.Id != "" {
					...
					for _, field := range tableFields {
						...
						if field.AutofillField != "" && autoFillTable != "" && autoFillTable == strings.Split(fieldReq.Id, "#")[0] {
							...
							fieldRes = append(fieldRes, field)
							if strings.Contains(fieldReq.Id, "@") {
								field.Id = fieldReq.Id
							} else {
								guid := fieldReq.Id
							...
							var fieldResResponse []*nb.FieldResponse
							for _, field := range fieldRes {
								fieldResResponse = append(fieldResResponse, &nb.FieldResponse{
									Id:         field.Id,
							...
							var fieldsWithPermissions []*nb.FieldResponse
							fieldsForSection := make([]*nb.FieldForSection, len(fieldsWithPermissions))
							for i, field := range fieldsWithPermissions {
								fieldsForSection[i] = &nb.FieldForSection{
									Id:              field.Id,
									Column:          field.Column,
